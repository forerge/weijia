			...
			<block v-for="(val,index) in tuijianContent" :key="index">
				<view class="grid grid-col-2 tuijian-content-list">
					<view class="grid-list grid-combine-col-2 ">
						<navigator class="navigator" url="../fangyuanxiangqing/fangyuanxiangqing" hover-class="none">
							...
							<view class="btn-box grid-row-align-space-around-center">
								<text class="btn" v-if="val.h_wuye == 1">物业未交接</text> 
								<text class="btn" v-else-if="val.h_wuye == 2">物业已交接</text> 
								<text class="btn active" v-if="val.o_status == 2">租房未确认</text>
								<text class="btn active" v-else-if="val.o_status == 3">租房已确认</text>
							</view>
						</navigator>
					</view> 
				</view>
...
<script>
	export default {
		...
		onLoad() {
			...
			uni.request({ 
				url: this.serverApiUrl+'home/order/kuai_order_wode', //请求url
				method: 'POST',               //请求方式 
				data: {
					u_id:this.uid,
					role:this.role
